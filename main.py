...
def build_model(vocab_size, device):
    return GPT(
        ...
        embed_dim=256,   # ← FIX
        num_heads=8,     # match training
        num_layers=4,
        block_size=64
    ).to(device)

def train(model, train_data, val_data, device, steps=1000):
    optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4)

    for step in range(steps):
        x, y = get_batch('train', train_data, val_data, 64, 32)
...
# 4. INFERENCE
# -----------------------------
def generate(model, encode, decode, device, start, max_new_tokens=300):
    model.eval()
...
def inference(model, encode, decode, device):
    model.load_state_dict(torch.load("model.pt", map_location=device))
    text = generate(model, encode, decode, device, start="Shubham Education")
    print("\nGenerated:\n")
    return text
...
if __name__ == "__main__":
    ...
    inference(model, encode, decode, device)